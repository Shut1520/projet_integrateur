...
- [x] **(B)** **6.2** Test d'intégration broker : publier une mesure MQTT **réelle (ESP32)** → vérifier son insertion en BD via le subscriber.
- [x] **(B)** **6.3** Flux commandes complet : le firmware ESP32 récupère une commande en attente, confirme l'exécution, crée une `Action` → vérifie le flux complet en BD. (validé 2026-09-05 : cmd #275 envoyée→recue→executee en 1.7s via MQTT push + pull)
...
| 2026-09-05 | **6.2 valide** | **Test d'integration broker MQTT -> BD avec le vrai ESP32.** Valide : ESP32 publie en MQTT TLS 8883 sur `sai/Serre A/capteurs/telemetrie`, subscriber backend insere en BD (`source='esp32'`). Capter 71 (DHT22) : 29.3 C / 62.8% hum air (valeurs reelles). Capteur 72 (YL-69) : 17-21% sec -> 10.9-11.3% trempe -> 0% retire (variation physique confirmee). Timestamps a jour (NTP sync). Variation physique documentee (trempage eau). | opencode |
| 2026-09-05 | **Fix decalage commandes** | **Diagnose + correctifs latence commandes.** (1) Diagnostic : pics transitoires 10-32s causes par HTTPClient ESP32 sans timeout explicite (defaut ~10s read) + hotspot iPhone instable. Latence nominale = pull INTERVALLE_COMMANDES 5s + traitement ~ms. Triple ON web observe (re-clics pendant latence physique). (2) Firmware : `setConnectTimeout(3000)` + `setTimeout(3000)` sur tous les HTTPClient dans http_commands.cpp ; INTERVALLE_COMMANDES 5000->2000 ms. Build PIO SUCCESS (RAM 14.5%, Flash 73.5%). (3) Backend : nouveau RATE_LIMIT_PULL=60/min sur GET /attente (sinon 429 avec pull 2s=30/min). Migration Alembic : historique_actions.id_utilisateur passe nullable (FK automation id=0 -> None). Fix automatisation_service.py : id_utilisateur=None au lieu de 0. (4) Frontend : bouton toggle desactive pendant envoi (pendingActId) dans Dashboard.jsx et Actionneurs.jsx, evite les multi-clics. (5) Tests : 131 passed, 0 failed. | opencode |
| 2026-09-05 | **Fix desync onglet Actionneurs** | **Souscription MQTT temps réel ajoutée à Actionneurs.jsx.** Le panneau Actionneurs chargeait uniquement via `GET /api/actionneurs` (state figé) : les changements d'état venant de l'ESP32 (automation capteur) ou du CLI n'étaient pas reflétés. Fix : import `subscribeActionneurs` depuis `frontend/src/services/mqtt.js` + nouveau `useEffect` calqué sur `Dashboard.jsx:347-358` (match `payload.nom` → `setActionneurs` par map, avec fallback `payload.commande === 'on' ? 'actif'`). Aucun changement ACL requis (sai_frontend read `sai/#` déjà OK). Build frontend : OK (31.86s). Tests backend : 131 passed, 0 failed. | opencode |
| 2026-09-05 | **HC-SR04 + buzzer alertes** | **Bascule niveau d'eau vers capteur ultrason HC-SR04 + ajout buzzer actif GPIO5.** (1) `pins.h` : remplacement GPIO_NIVEAU_EAU (ADC) par GPIO_ULTRASO_TRIG=32 + GPIO_ULTRASO_ECHO=33 (diviseur 5V->3.3V sur ECHO). (2) `sensors.cpp` : constants HAUTEUR_RECIPIENT_MM=200, HAUTEUR_PLEIN_MM=180 ; `read_water_level()` utilise `pulseIn(ECHO, HIGH, 5000)` → distance_mm → hauteur = 200−distance → pct = (hauteur/180)*100 clampé [0,100] ; `sensors_begin()` : retrait ADC, ajout pinMode TRIG/OUTPUT + ECHO/INPUT. (3) Nouveau module `buzzer.h/.cpp` : buzzer actif GPIO5, `buzzer_beep(nb_bips, duree_ms)` non-bloquant (millis). (4) `main.cpp` : buzzer_begin() au setup + buzzer_loop() dans loop. (5) `automation.cpp` : 3 bips longs (500 ms) sur perte liaison ; 1 bip court (200 ms) sur activation pompe/ventilation par seuil. (6) `README.md` : table câblage mise à jour (HC-SR04 + buzzer), note géométrie capteur, tests étendus. Seed BD déjà HC-SR04 pour niveau_eau → aucun changement backend/frontend/CLI. Build PIO + flash à faire. | opencode |
| 2026-09-05 | **Optimisation latence commandes** | **Latence commandes→actionneur optimisée (fast-path MQTT push + réglages réseau).** (1) Diagnostic : pull HTTP 2s (plancher ~1s), timeouts HTTP 3s sur le même thread que MQTT keepalive → dégradation auto sur hotspot congestionné, WiFi modem-sleep = latence paquets variable. (2) Firmware : `WiFi.setSleep(false)` (`wifi_manager.cpp`) ; timeouts HTTP 3000→1200ms (3 endroits `http_commands.cpp`) ; `mqtt_client.setKeepAlive(30)` (`mqtt_publisher.cpp`) ; `INTERVALLE_COMMANDES 2000→1300` (~46/min, sous rate limit 60/min). (3) Fast-path MQTT push : backend `publier_commande_notification()` (`mqtt_service.py`) publie `sai/<parcelle>/commandes/notif` à la création d'une commande ; ESP32 souscrit au topic et appelle `http_commands_pull_maintenant()` (flag `pull_immediat` en IDLE) → pull immédiat (~100-400ms vs ~1s). ACL : `sai_backend` write, `sai_esp32` read sur `sai/+/commandes/notif`. (4) Frontend : `Promise.all` dans `handleToggle` (`Actionneurs.jsx`) → 2 RTT séquentiels → 1. (5) Build PIO SUCCESS (RAM 14.5%, Flash 73.7%), flash COM10 OK, 131 tests passed. | opencode |
| 2026-09-05 | **Reload code/ACL + latence prouvée** | **Diagnostic : backend/broker stale = cause racine latence > 5s.** Backend (PID 7988 démarré 14:48) exécutait l'ancien code (sans `publier_commande_notification`, modifié 18:47-18:49) ; broker (PID 6572 démarré 11:09) avait l'ancienne ACL (modifiée 18:47) → notif MQTT jamais émise en service. **Correctifs :** (1) `_publisher_client()` : timeout connect 5s (au lieu de 30s par défaut), `is_connected()` check, cleanup/retry sur client déconnecté, null-safety dans `publier_commande_notification`/`publier_alerte`. (2) `main.py` : pre-chauffage du publisher au démarrage (évite blocage 30s au 1er appel). (3) Redémarrage broker (PID 8472) + backend (PID 18220) avec code/ACL à jour. **Mesure latence bout en bout : 1.7s** (création → `executee`, polling 500ms). Pull ESP32 ~1.1s (dans l'intervalle 1300ms), 4 appels HTTP séquentiels ~0.6s. **Phase C non nécessaire** (cible < 5s atteinte). 131 tests passed, 0 failed. | opencode |
| 2026-09-06 | **Push MQTT notif prouvé + nettoyage** | **Push MQTT notif confirmé.** (1) Fix bug : `Client.connect()` rejetait `timeout`/`connect_timeout` (paho-mqtt v2 ne supporte pas ce kwarg) → paramètre supprimé. (2) Backend relancé sans `--reload` → un seul subscriber MQTT (PID 900 orphelin = spawn child uvicorn supprimé). (3) Preuve push : `mosquitto_sub` sur `sai/+/commandes/notif` reçoit la notif à la création d'une commande → ESP32 pull immédiat. (4) Old mosquitto PID 4708 (127.0.0.1:1883,.Service Windows orphelin) tué. (5) Commandes orphelines 271/272 nettoyées. (6) `server.log`/`server_err.log` ajoutés au `.gitignore`. **Lot 2 différé (batch firmware)** : A3 (clé API NVS), A1-serial (preuve COM10), C6 (cert SAN + retirer setInsecure). | opencode |
| 2026-09-06 | **Étapes 2-3-4 IoT finalisées** | **Backend automation + firmware + confort.** (1) **Étape 2 backend** : `automatisation_service.py` — dedoublonnage alertes (`_alerte_deja_active`), résolution auto (`_resoudre_alertes`), commande cohérente (`creer_commande` source="auto"), compteur `alertes_resolues` dans `executer_boucle`. Tests : 15 nouveaux dans `test_automatisation.py` (dedoublon, résolution, évaluation, boucle) — tous passent. (2) **Étape 3 firmware** : hystérésis seuils (`SEUIL_SOL_REACTIV` 33%, `SEUIL_TEMP_REACTIV` 37°C, `SEUIL_CO2_REACTIV` 850 ppm), seuil éclairage (`SEUIL_LUM_BAS` 30%, `SEUIL_LUM_HAUT` 50%) ajoutés à `config.h` + `automation.cpp`. Fix `extern void` → `#include "http_commands.h"` dans `mqtt_publisher.cpp`. Build + flash COM10 OK, boot ESP32 confirmé (WiFi, NTP, MQTT TLS, subscribe notif, telemetry). (3) **Étape 4 confort** : `DEVICE_ID` déplacé de `mqtt_publisher.cpp` vers `config.h` (configurable). Tests backend : 143 passed, 3 failed (pré-existants 401/403). | opencode |