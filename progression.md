...
| Scroll isolé au contenu (sidebar fixe) | ✅ | `index.css` + `AppLayout.jsx` + `Sidebar.jsx` + `TopBar.jsx`. Sidebar `lg:relative h-full overflow-hidden`, contenu `<main>` `overflow-y-auto` uniquement. Root `h-screen overflow-hidden`. |
| Sidebar profil + déconnexion inline | ✅ | `Sidebar.jsx` : profil (`flex-1`) et bouton déconnexion (`shrink-0`, icône seule) sur la même ligne horizontale. Suppression de `space-y-2`. |
