...
- **Scroll layout** : Dans une app dashboard avec sidebar, utiliser `h-screen overflow-hidden` sur le root + `overflow-y-auto` uniquement sur `<main>`. La sidebar doit être `h-full overflow-hidden` (pas `static` qui laisse le scroll global). Voir `AppLayout.jsx`, `Sidebar.jsx`, `TopBar.jsx`.
