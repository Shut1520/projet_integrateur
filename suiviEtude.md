...
- **KPIs interactives** : `GaugeCard` accepte une prop `onClick`. Le Dashboard passe `onClick={() => navigate('/history?capteur=xxx')}`. History.jsx lit `searchParams.get('capteur')` via `useSearchParams()`, puis fait un match insensible à la casse sur `cap.nom` pour pré-sélectionner le filtre. Voir `Dashboard.jsx` (lignes des GaugeCard) + `History.jsx:L54-78`.
- **Toggle interrupteur** : Le toggle dans le Dashboard (contrôle rapide) utilise un `<button>` CSS (pas un `<input type="checkbox">`). `handleToggleActuator` fait un double appel API : `updateActionneur(id, {etat})` PUIS `commanderActionneur(id, action)`. Sans le premier appel, l'état ne serait pas persisté en BDD. Voir `Dashboard.jsx:L163-180`.
- **Filtres CRUD** : Pattern unifié sur toutes les pages : barre `bg-white` avec `Filter` icon + selects (parcelle, etat, type_culture, proprietaire) + `Search` input. Le filtrage est fait côté client avec `useMemo`. Toujours `String()` pour comparer les IDs des selects (string) avec les IDs des objets (number). Voir les 3 fichiers `Parcelles.jsx`, `Actionneurs.jsx`, `Capteurs.jsx`.
...
- **Scroll layout** : Dans une app dashboard, utiliser `h-screen overflow-hidden` sur le root + `overflow-y-auto` uniquement sur `<main>`. La sidebar doit être `h-full overflow-hidden` (pas `static` qui laisse le scroll global). Voir `AppLayout.jsx`, `Sidebar.jsx`, `TopBar.jsx`.
- **Sidebar layout bottom** : Pour économiser l'espace vertical en bas de sidebar, empiler horizontalement (`flex items-center`) le profil (`flex-1`) et le bouton déconnexion (`shrink-0`, icône seule). Voir `Sidebar.jsx`.
